...
//map kernel pagetable from user user pagetable
int
kvmmap_fromp(pagetable_t pagetable, pagetable_t kpagetable, uint64 from, uint64 to){
    pte_t *user_pte;
    pte_t *kernel_pte;
    uint64 pa, i;
    uint flags;
    ...
    for(i = from; i < to; i += PGSIZE){
        if((user_pte = walk(pagetable, i, 0)) == 0)
            panic("kvmmap_fromp: upagetable pte should exist");
        if((kernel_pte = walk(kpagetable, i, 1)) == 0)
            panic("kvmmap_fromp: kernel_pte walk error");
        if((*user_pte & PTE_V) == 0)
            panic("kvmmap_fromp: page not present");
        pa = PTE2PA(*user_pte);
        flags = PTE_FLAGS(*user_pte);
        flags &= ~PTE_U;
